...
HOME = Path.home()
# Five user-defined scan roots — searched recursively (rglob, unlimited depth)
# and deduped by resolved physical path so symlinked overlaps don't double up.
DEFAULT_GGUF_ROOTS = [
    Path("<Your Model Directory>"),
    Path("<Your Model Directory>"),
    Path("/Volumes/SamsungSSDE/.cache/huggingface"),
...
def main():
    ...
    # Resolve source roots: user-supplied (if any) else defaults, keep only existing ones
    candidate_roots = args.gguf_root if args.gguf_root else DEFAULT_GGUF_ROOTS
    source_roots = [r for r in candidate_roots if r.exists()]
    if not source_roots:
